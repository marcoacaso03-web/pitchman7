                ...
                <Card className="bg-card dark:bg-black/40 border-border dark:border-brand-orange/30 shadow-sm dark:shadow-[0_0_15px_rgba(172,229,4,0.05)] rounded-3xl overflow-hidden backdrop-blur-sm transition-colors">
                    ...
                    <CardContent>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <div className="flex flex-col items-center p-3 bg-muted/50 dark:bg-black/40 rounded-2xl border border-primary/20 dark:border-brand-orange/20 group hover:border-primary/40 dark:hover:border-brand-orange/40 transition-all">
                                <span className="text-[10px] font-black text-primary dark:text-brand-orange uppercase tracking-widest mb-1.5 opacity-80">VITTORIA</span>
                                <div className="flex items-baseline gap-1">
                                    <span className="text-2xl font-black text-foreground dark:text-white">{teamRecord.wins}</span>
                                    <span className="text-xs font-bold text-primary/60 dark:text-brand-orange/60">({winPct}%)</span>
                                </div>
                            </div>
                    ...
                    <CardContent>
                        <div className="flex items-end justify-between mb-6">
                            <div className="space-y-1">
                                <div className="flex items-center gap-3">
                                    <span className={`text-4xl font-black ${goalDifference > 0 ? 'text-primary dark:text-brand-orange' : goalDifference < 0 ? 'text-rose-500' : 'text-foreground dark:text-white'}`}>
                                        {goalDifference > 0 ? `+${goalDifference}` : goalDifference}
                                    </span>
                                    {goalDifference > 0 ? <TrendingUp className="h-7 w-7 text-primary dark:text-brand-orange drop-shadow-[0_0_8px_rgba(172,229,4,0.4)]" /> : goalDifference < 0 ? <TrendingDown className="h-7 w-7 text-rose-500 drop-shadow-[0_0_8px_rgba(244,63,94,0.4)]" /> : null}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                            <div className="flex items-start gap-3">
                                <div className="p-2 bg-primary/10 dark:bg-brand-orange/10 rounded-xl border border-primary/20 dark:border-brand-orange/20">
                                    <GiSoccerBall className="h-4 w-4 text-primary dark:text-brand-orange" />
                                </div>
                                <div>
